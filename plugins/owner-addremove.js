...
if (!coinTxt) return m.reply(`ꕥ Por favor, ingresa la cantidad que deseas añadir.`)
if (isNaN(coinTxt)) return m.reply(`ꕥ Solo se permiten números.`)
await m.react('🕒')
...
if (!xpTxt) return m.reply(`ꕥ Ingresa la cantidad de experiencia (XP) que deseas añadir.`)
if (isNaN(xpTxt)) return m.reply(`ꕥ Solo números son permitidos.`)
await m.react('🕒')
